...
class U_net(nn.Module):
    ...
    def __init__(self, in_channel, n_classes):
        ...
        self.cont_layer1 = nn.Sequential(OrderedDict([
           ...
           ('c_act_f11', nn.ReLU(inplace=True)),
           ...
           ('c_act_f12', nn.ReLU(inplace=True)),
        ]))
        ...
        self.cont_layer2 = nn.Sequential(OrderedDict([
            ...
            ('c_act_f21', nn.ReLU(inplace=True)),
            ...
            ('c_act_f22', nn.ReLU(inplace=True))
        ...
        self.cont_layer3 = nn.Sequential(OrderedDict([
            ...
            ('c_act_f31', nn.ReLU(inplace=True)),
            ('c_bn_32', self.c_bn32),
            ...
            ('c_act_f32', nn.ReLU(inplace=True))
        ]))
        # 4th layer
        self.cont_layer4 = nn.Sequential(OrderedDict([
            ...
            ('c_act_f41', nn.ReLU(inplace=True)),
            ('c_bn_42', self.c_bn42),
            ...
            ('c_act_f42', nn.ReLU(inplace=True))
        ]))
        # 5th layer
        self.cont_layer5 = nn.Sequential(OrderedDict([
            ...
            ('c_act_f51', nn.ReLU(inplace=True)),
            ...
            ('c_act_f52', nn.ReLU(inplace=True))
        ]))
...
class U_net_P(nn.Module):
    ...
    def __init__(self, in_channel, n_classes):
        ...
        self.cont_layer1 = nn.Sequential(OrderedDict([
            ('c_conv11',
             nn.Conv2d(in_channels=self.in_channel, out_channels=self.n_out_channels1, kernel_size=self.c_kernel_size)),
            ('c_bn_11', self.c_bn11),
            ('c_act_f11', self.c_activation_f),
            # ('c_act_f11', nn.ReLU(inplace=True)),
            ('c_conv12', nn.Conv2d(in_channels=self.n_out_channels1, out_channels=self.n_out_channels1,
                                   kernel_size=self.c_kernel_size)),
            ('c_bn_12', self.c_bn12),
            ('c_act_f12', self.c_activation_f)
            # ('c_act_f12', nn.ReLU(inplace=True)),
        ]))
        ...
        # 2th layer
        self.cont_layer2 = nn.Sequential(OrderedDict([
            ('c_conv21', nn.Conv2d(in_channels=self.n_out_channels1, out_channels=self.n_out_channels2,
                                   kernel_size=self.c_kernel_size)),
            ('c_bn_21', self.c_bn21),
            ('c_act_f21', self.c_activation_f),
            # ('c_act_f21', nn.ReLU(inplace=True)),
            ('c_conv22', nn.Conv2d(in_channels=self.n_out_channels2, out_channels=self.n_out_channels2,
                                   kernel_size=self.c_kernel_size)),
            ('c_bn_22', self.c_bn22),
            ('c_act_f22', self.c_activation_f)
            # ('c_act_f22', nn.ReLU(inplace=True))
        ...
        # 3th layer
        self.cont_layer3 = nn.Sequential(OrderedDict([
            ('c_conv31', nn.Conv2d(in_channels=self.n_out_channels2, out_channels=self.n_out_channels3,
                                   kernel_size=self.c_kernel_size)),
            ('c_bn_31', self.c_bn31),
            ('c_act_f31', self.c_activation_f),
            # ('c_act_f31', nn.ReLU(inplace=True)),
            ('c_bn_32', self.c_bn32),
            ('c_conv32', nn.Conv2d(in_channels=self.n_out_channels3, out_channels=self.n_out_channels3,
                                   kernel_size=self.c_kernel_size)),
            ('c_act_f32', self.c_activation_f)
            # ('c_act_f32', nn.ReLU(inplace=True))
        ]))
        # 4th layer
        self.cont_layer4 = nn.Sequential(OrderedDict([
            ('c_conv41', nn.Conv2d(in_channels=self.n_out_channels3, out_channels=self.n_out_channels4,
                                   kernel_size=self.c_kernel_size)),
            ('c_bn_41', self.c_bn41),
            ('c_act_f41', self.c_activation_f),
            # ('c_act_f41', nn.ReLU(inplace=True)),
            ('c_bn_42', self.c_bn42),
            ('c_conv42', nn.Conv2d(in_channels=self.n_out_channels4, out_channels=self.n_out_channels4,
                                   kernel_size=self.c_kernel_size)),
            ('c_act_f42', self.c_activation_f),
            # ('c_act_f42', nn.ReLU(inplace=True))
        ]))
        # 5th layer
        self.cont_layer5 = nn.Sequential(OrderedDict([
            ('c_conv51', nn.Conv2d(in_channels=self.n_out_channels4, out_channels=self.n_out_channels5,
                                   kernel_size=self.c_kernel_size)),
            ('c_bn_51', self.c_bn51),
            ('c_act_f51', self.c_activation_f),
            # ('c_act_f51', nn.ReLU(inplace=True)),
            ('c_conv52', nn.Conv2d(in_channels=self.n_out_channels5, out_channels=self.n_out_channels5,
                                   kernel_size=self.c_kernel_size)),
            ('c_bn_52', self.c_bn52),
            ('c_act_f52', self.c_activation_f)
            # ('c_act_f52', nn.ReLU(inplace=True))
        ...
        # 4th layer
        self.exp_layer4 = nn.Sequential(OrderedDict([
            ('e_conv41', nn.Conv2d(in_channels=self.n_skip_channels4, out_channels=self.n_out_channels4,
                                   kernel_size=self.c_kernel_size)),
            ('e_act_f41', self.c_activation_f),
            ('e_conv42', nn.Conv2d(in_channels=self.n_out_channels4, out_channels=self.n_out_channels4,
                                   kernel_size=self.c_kernel_size)),
            ('e_act_f42', self.c_activation_f),
            ('e_up_conv4', nn.ConvTranspose2d(in_channels=self.n_out_channels4, out_channels=self.n_out_channels3,
                                              kernel_size=self.up_kernel_size, stride=self.deconv_stride))
        ...
        # 3th layer
        self.exp_layer3 = nn.Sequential(OrderedDict([
            ('e_conv31', nn.Conv2d(in_channels=self.n_skip_channels3, out_channels=self.n_out_channels3,
                                   kernel_size=self.c_kernel_size)),
            ('e_act_f31', self.c_activation_f),
            ('e_conv32', nn.Conv2d(in_channels=self.n_out_channels3, out_channels=self.n_out_channels3,
                                   kernel_size=self.c_kernel_size)),
            ('e_act_f32', self.c_activation_f),
            ('e_up_conv3', nn.ConvTranspose2d(in_channels=self.n_out_channels3, out_channels=self.n_out_channels2,
                                              kernel_size=self.up_kernel_size, stride=self.deconv_stride))
        ...
        # 2th layer
        self.exp_layer2 = nn.Sequential(OrderedDict([
            ('e_conv21', nn.Conv2d(in_channels=self.n_skip_channels2, out_channels=self.n_out_channels2,
                                   kernel_size=self.c_kernel_size)),
            ('e_act_f21', self.c_activation_f),
            ('e_conv22', nn.Conv2d(in_channels=self.n_out_channels2, out_channels=self.n_out_channels2,
                                   kernel_size=self.c_kernel_size)),
            ('e_act_f22', self.c_activation_f),
            ('e_up_conv2', nn.ConvTranspose2d(in_channels=self.n_out_channels2, out_channels=self.n_out_channels1,
                                              kernel_size=self.up_kernel_size, stride=self.deconv_stride))
        ...
        # 1th layer
        self.exp_layer1 = nn.Sequential(OrderedDict([
            ('e_conv11', nn.Conv2d(in_channels=self.n_skip_channels1, out_channels=self.n_out_channels1,
                                   kernel_size=self.c_kernel_size)),
            ('e_act_f11', self.c_activation_f),
            ('e_conv12', nn.Conv2d(in_channels=self.n_out_channels1, out_channels=self.n_out_channels1,
                                   kernel_size=self.c_kernel_size)),
            ('e_act_f12', self.c_activation_f),
            ('e_conv_f', nn.Conv2d(in_channels=self.n_out_channels1, out_channels=self.final_out_channels,
                                   kernel_size=self.final_kernel_size))
...
if __name__ == '__main__':
    ...
    u_net = U_net(1,1)
    u_net_p = U_net_P(1,1)
    x = torch.rand(2,1,255, 255)
    u_net.eval()
    u_net_p.eval()
    # torch.save()
    with torch.no_grad():
        s = u_net.state_dict()
        p = u_net_p.state_dict()
        torch.save(s, 'sd_n')
        torch.save(p, 'sd_p')
        print(f's:{u_net(x)} \np:{u_net_p(x)}')
        summary(u_net, (1,255,255), device='cpu')
        summary(u_net_p, (1, 255, 255), device='cpu')
